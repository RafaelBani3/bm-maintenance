    ...
    $(document).ready(function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success && response.data.uom.length > 0) {
                    ...
                    initSelect2Unit(); // Inisialisasi awal
                } else {
        ...
        $('#add-row').on('click', function () {
            ...
            updateRowNumbers();
            initSelect2Unit(); // Inisialisasi Select2 di row baru
        });
    ...
    {{-- <script>
        async function fetchUOMOptions() {
            try {
                ...
                if (data.success && Array.isArray(data.data.uom)) {
                    const optionsHtml = data.data.uom.map(uom =>
                        `<option value="${uom.descs}" data-code="${uom.uom_cd}">${uom.uom_cd} - ${uom.descs}</option>`
                    ).join('');

                    // Populate semua select.unit yang sudah ada (pada edit mode)
                    document.querySelectorAll('.select2-unit').forEach(select => {
                        ...
                        select.innerHTML = `<option value="">Select Unit</option>` + optionsHtml;

                        // Select default if matched
                        if (selectedCode && selectedName) {
                            select.value = selectedName;
                            const hidden = select.parentElement.querySelector('input[type=hidden]');
                            const selectedOpt = [...select.options].find(opt => opt.dataset.code === selectedCode);
                            if (selectedOpt) {
                                select.value = selectedOpt.value;
                                if (hidden) hidden.value = selectedOpt.dataset.code;
                            }
                        }

                        // Init Select2
                        $(select).select2({ placeholder: "Select Unit", width: '100%' });
                    });
                }
            } catch (error) {
        ...
        // Update hidden input when unit changes
