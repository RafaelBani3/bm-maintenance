...
class CatSeeder extends Seeder
{
    ...
    {
        // Insert Categories menggunakan Model agar event boot() berjalan
        $categories = [
            'New Installation',
            'Repair',
            'Licenses',
        ...
        $catIds = []; // Array untuk menyimpan Cat_No yang dibuat

        foreach ($categories as $category) {
            $cat = Cats::create(['Cat_Name' => $category]);
            $catIds[$category] = $cat->Cat_No; // Simpan Cat_No berdasarkan nama kategori
        }

        // Insert Subcategories dengan referensi Cat_No dari hasil insert sebelumnya
        $subcategories = [
