...
class technician extends Model
{
    ...
    public static function getIncrementTechnicianNo()
    {
        $currentYear = date('Y');
        $currentMonth = date('m');

        $lastNumber = self::whereYear('created_at', $currentYear)
            ->whereMonth('created_at', $currentMonth)
            ->select(DB::raw("IFNULL(MAX(CAST(TRIM(LEADING '0' FROM SUBSTR(technician_id, 11, 3)) AS UNSIGNED)), 0) AS max_n"))
            ->first()
            ->max_n;
